...
def fetch_dataloader(types, data_dir, params):
    ...
    for split in ['train', 'val', 'test']:
        if split in types:
            ...
            if split == 'train':
                dl = DataLoader(HOUSEDataset(path, train_transformer), batch_size=params.batch_size, shuffle=True,
                                        num_workers=params.num_workers,
                                        pin_memory=params.cuda)
            else:
                dl = DataLoader(HOUSEDataset(path, eval_transformer), batch_size=params.batch_size, shuffle=False,
                                num_workers=params.num_workers,
                                pin_memory=params.cuda)
